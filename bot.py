...
# Physical (ro)Bot that interacts with the environment.

# An agent asks the “bot” to
#	- observe state and report changed values.
#	- choose an enabled action to perform, and
#	- perform ab action and return only when action is completed.
...
class Bot:
    ...
    def script_select_enabled_action(self, enabled_actions):
        # Returns action in enabled_actions whose operator is specified by the current stage of bot's script.
        current_stage = self.execution_scenario.encoded_stages[self.index]
    ...
    def user_select_enabled_action(enabled_actions: list[ss.Action])-> ss.Action or None:
        ...
        while True:
            ...
            if utils.index_in_rangep(selected_i, 0, max_i):
                break
            else:
                print(f"{selected_i} not an index in [0, {max_i}].")
        return enabled_actions[int(selected_i)]

    def perform_action(self, action: ss.Action):
        # Tell bot to perform action. Optionally wait to confirm completion.
